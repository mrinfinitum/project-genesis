    ...
    <main className="mx-auto w-full max-w-[1800px] px-5 py-6 lg:px-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[248px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 rounded-lg border border-cyan-300/20 bg-slate-950/45 p-5 lg:p-7">
          ...
          {section === "components" && (
            <div className="space-y-6"><SectionTitle eyebrow="Component Library" title="Versioned building blocks" detail="The NOVERIS Component Library is the authoritative catalog for published component contracts, live states, previews, Unity prefab identifiers, and screen usage." /><div className="rounded-md border border-cyan-300/20 bg-slate-950/55 p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="font-semibold text-slate-100">Open the canonical component workspace</p><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-400">Component contracts inherit from this Design Language. Unity consumes token references and prefab IDs without copying local screen styling.</p></div><Link href="/creative-production/design-system/component-library" className="inline-flex items-center gap-2 rounded-md border border-cyan-300/35 bg-cyan-300/10 px-3 py-2 text-sm font-semibold text-cyan-50 hover:bg-cyan-300/20"><Component className="h-4 w-4" />Open Component Library</Link></div></div></div>
          )}
