...
import { toIdentityRelationshipRuntimeExport, type IdentityRelationshipRuntimeExport } from "@/lib/identity-relationships";
import {
...
const targetConfigs: Record<EngineTarget, EngineTargetConfig> = {
  ...
  unity: {
    ...
    generatedModules: ["ResourceCatalog.cs", "ResearchUnlocks.cs", "NoverisDesignLanguage.json", "NoverisComponentLibrary.json", "NoverisScreenTemplateLibrary.json", "NoverisAssetProductionRuntime.json", "NoverisIdentityRelationships.json", "UniverseLoader.cs"],
  ...
  unreal: {
    ...
    generatedModules: ["ResourceCatalog", "ResearchUnlockTable", "NoverisDesignLanguage.json", "NoverisComponentLibrary.json", "NoverisScreenTemplateLibrary.json", "NoverisAssetProductionRuntime.json", "NoverisIdentityRelationships.json", "UniverseData"],
  ...
  godot: {
    ...
    generatedModules: ["ResourceCatalog.gd", "ResearchUnlocks.gd", "NoverisDesignLanguage.json", "NoverisComponentLibrary.json", "NoverisScreenTemplateLibrary.json", "NoverisAssetProductionRuntime.json", "NoverisIdentityRelationships.json", "UniverseLoader.gd"],
  ...
  web: {
    ...
    generatedModules: ["project-genesis.types.ts", "NoverisDesignLanguage.json", "NoverisComponentLibrary.json", "NoverisScreenTemplateLibrary.json", "NoverisAssetProductionRuntime.json", "NoverisIdentityRelationships.json", "projectGenesisClient.ts", "projectGenesisStore.ts"],
  ...
  generic: {
    ...
    generatedModules: ["canonical-data.json", "design-language.json", "component-library.json", "screen-template-library.json", "asset-production-runtime.json", "identity-relationships.json", "schema-notes.json", "relationship-map.json"],
...
type CanonicalModules = {
  ...
  identity_relationships: IdentityRelationshipRuntimeExport;
...
async function buildCanonicalModules(data: GameData): Promise<CanonicalModules> {
  ...
  const runtimeAssets = runtime.assetProductionRuntime;
  if (!runtime.identityRelationshipGraph) {
    throw new Error("Canonical identity and relationship graph is required before engine exports can be generated.");
  }
  ...
  return {
    ...
    identity_relationships: toIdentityRelationshipRuntimeExport(runtime.identityRelationshipGraph),
...
function buildRelationshipMap(modules: CanonicalModules) {
  ...
  return {
    ...
    economyUsage,
    canonicalIdentity: {
      id: modules.identity_relationships.id,
      version: modules.identity_relationships.version,
      status: modules.identity_relationships.status,
      parentByRecord: Object.fromEntries(modules.identity_relationships.records.map((record) => [record.canonicalId, record.parentCanonicalId])),
      childrenByRecord: Object.fromEntries(modules.identity_relationships.records.map((record) => [record.canonicalId, record.childCanonicalIds]))
    }
  };
}
...
function validateEngineExport(target: EngineTarget, modules: CanonicalModules) {
  ...
  const identityRecords = new Set(modules.identity_relationships.records.map((record) => record.canonicalId));
  if (modules.identity_relationships.id !== "noveris-identity-relationships" || modules.identity_relationships.status !== "Ready") {
    addIssue(issues, "error", "identity_relationships_invalid", "Engine exports require a Ready canonical identity and relationship graph.", [modules.identity_relationships.id, modules.identity_relationships.status]);
  }
  if (identityRecords.size !== modules.identity_relationships.records.length) {
    addIssue(issues, "error", "identity_relationship_duplicate_id", "Identity runtime records must use globally unique canonical IDs.");
  }
  const brokenIdentityRelationships = modules.identity_relationships.relationships.filter((relationship) => !identityRecords.has(relationship.fromCanonicalId) || !identityRecords.has(relationship.toCanonicalId));
  if (brokenIdentityRelationships.length) {
    addIssue(issues, "error", "identity_relationship_broken_reference", "Identity runtime relationships must resolve canonical IDs.", brokenIdentityRelationships.map((relationship) => `${relationship.fromCanonicalId}:${relationship.toCanonicalId}`));
  }
  const brokenIdentityParents = modules.identity_relationships.records.filter((record) => record.parentCanonicalId && !identityRecords.has(record.parentCanonicalId));
  if (brokenIdentityParents.length) {
    addIssue(issues, "error", "identity_relationship_missing_parent", "Identity runtime parent references must resolve canonical IDs.", brokenIdentityParents.map((record) => record.canonicalId));
  }
...
function compactModules(modules: CanonicalModules) {
  return {
    ...
    identity_relationships: modules.identity_relationships,
...
function targetArtifacts(target: EngineTarget, modules: CanonicalModules) {
  if (target === "roblox") {
    return {
      ...
      "IdentityRelationshipsModule.lua": `local IdentityRelationships = ${luaValue(modules.identity_relationships)}\n\nreturn IdentityRelationships\n`,
  ...
  if (target === "web") {
    return {
      ...
      "NoverisIdentityRelationships.json": modules.identity_relationships,
  ...
  return {
    ...
    "identity-relationships.json": modules.identity_relationships,
