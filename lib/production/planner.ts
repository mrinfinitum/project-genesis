...
function buildReports(data: GameData, assetState: AssetProductionState, authoringState?: ContentAuthoringState, screenState?: ScreenDesignerState, componentState?: ComponentLibraryState, upgradeArtReport?: UpgradeArtReport, aiAgentState?: AiAgentLibraryState, architectureState?: ArchitectureState): ProductionReport[] {
  ...
  return [
    ...
    { label: "Resource Economy Contracts", count: Math.max(0, primaryHudEconomyIds.length - contracts.length), href: "/economy-designer", severity: contracts.length === primaryHudEconomyIds.length ? "Low" : "Critical", description: "Permanent HUD economies missing complete behavior contracts." },
    { label: "Building Resource Effects", count: impliedMissingEffects, href: "/economy-designer", severity: impliedMissingEffects ? "High" : "Low", description: "Buildings whose names imply production/capacity but lack structured resource effects." },
    { label: "Premium Crystal Unsafe Sources", count: premiumUnsafeSources, href: "/economy-designer", severity: premiumUnsafeSources ? "Critical" : "Low", description: "Premium Crystal producers that are generic building/offline sources." },
    { label: "Offline Progression Coverage", count: offlineCoverageGaps, href: "/economy-designer", severity: offlineCoverageGaps ? "High" : "Low", description: "Permanent HUD economies missing offline progression policy coverage." },
    { label: "Draft Era Scaffolds", count: draftScaffoldCount, href: "/era-starter-kits", severity: draftScaffoldCount ? "Medium" : "Low", description: "Generated era starter kits waiting for review and promotion." },
...
function scaffoldWorkItems(authoringState?: ContentAuthoringState) {
  ...
  return authoringState.scaffolds.flatMap((scaffold) =>
    productionTasksForScaffold(scaffold).slice(0, 18).map((task) => queueItem({
      ...
      href: "/era-starter-kits",
      reason: `${scaffold.eraName} draft was generated by the Era Starter Kits workflow.`,
...
export function buildProductionPlan(data: GameData, assetState: AssetProductionState, eraSummary: EraArtSummaryByEra, authoringState?: ContentAuthoringState, screenState?: ScreenDesignerState, componentState?: ComponentLibraryState, aiAgentState?: AiAgentLibraryState, architectureState?: ArchitectureState): ProductionPlan {
  ...
  const metrics: ProductionMetric[] = [
    ...
    { label: "Economy Designer Readiness", complete: resourceEconomyComplete, total: resourceEconomyTotal, value: percent(resourceEconomyComplete, resourceEconomyTotal), detail: `${resourceEconomyContracts.length}/${primaryHudEconomyIds.length} HUD contracts, ${resourceEconomyProducers.length} producers, ${resourceEconomyEffects.length} effects visible in Economy Designer.` },
