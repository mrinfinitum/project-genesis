...
function workspaceLayout(label: string): ScreenLayoutSpec {
  ...
  return {
    ...
    layoutMode: "hud_overlay",
    panelBounds: [],
    columns: `${label} visual layout is owned by the Game repository. Studio tracks reference screenshots and implementation contracts only.`,
    rows: "No authored game-screen panel rows. Persistent HUD and navigation are inherited from the shell.",
    spacing: "Client implementations own exact spacing, CSS/layout, responsive composition, and animation placement.",
    alignment: "Use uploaded reference screenshots for visual review; do not publish Studio-authored workspace coordinates.",
    safeAreas: ["Main Workspace Slot reference only"],
    overflowBehavior: "Game clients decide scroll, pan, drawers, and modal behavior from their implementation.",
    backgroundLayers: ["Reference screenshot when uploaded"],
    overlayLayers: ["Future screenshot annotations"]
  };
}
...
const researchMasterLayout: ScreenLayoutSpec = {
  ...
  layoutMode: "hud_overlay",
  panelBounds: [],
  columns: "Research visual composition is owned by the Game repository. Studio stores reference screenshots, component contracts, data requirements, asset keys, states, and handoff notes.",
  rows: "No Studio-authored research tree, branch sidebar, detail panel, or timeline geometry is generated.",
  spacing: "Client implementations own spacing and responsive layout. Studio validates contracts only.",
  alignment: "Use the uploaded/current reference screenshot for parity discussion; do not mimic the game inside Studio.",
  safeAreas: ["Main Workspace Slot reference only"],
  overflowBehavior: "Game clients decide screen scrolling, panning, drawers, and modal behavior.",
  backgroundLayers: ["Research reference screenshot when uploaded"],
  overlayLayers: ["Future screenshot annotations"]
};
...
function researchComponent(input: {
  ...
  return {
    ...
    variant: "reference-contract",
    state: "Default",
    layoutOverride: "Client implementations own exact layout geometry. Studio preserves reference screenshots and contract metadata only.",
    assetOverride: "Pending Upload",
    dataBindings: input.dataInputs ?? commonData,
    screenSpecificNotes: input.notes ?? "Reference contract only. Final Game UI layout is implemented in the Game repository while Studio preserves bindings, states, interactions, and asset keys.",
    displayName: input.displayName,
...
const researchComponents: ScreenComponentSpec[] = [
  ...
  researchComponent({
    ...
    purpose: "Research background asset requirement linked to the reference screenshot workflow.",
    dimensions: "Client-owned implementation dimensions.",
    positioning: "Owned by the Game repository; Studio tracks the asset key and readiness.",
  ...
  researchComponent({
    ...
    purpose: "Repeated branch row contract for canonical research categories and progress state.",
    dimensions: "Client-owned repeated row dimensions.",
    positioning: "Client-owned list placement.",
  ...
  researchComponent({
    ...
    purpose: "Data-driven selected branch title, description, hero/background art, progress label, percentage, and progress bar.",
    dimensions: "Client-owned header dimensions.",
    positioning: "Client-owned placement inside the research screen.",
  ...
  researchComponent({
    ...
    dimensions: "Client-owned node dimensions.",
    positioning: "Client-owned node placement derived from the Game implementation.",
  ...
  }),
  {
    ...baseRecord({
      ...
      notes: [
        ...
        "Reference controls required: upload current game screenshot, show/hide, annotation notes, and difference mode if supported.",
        "The Game repository owns the final Research screen layout, CSS, responsive composition, and animation placement.",
        "Studio preserves reference screenshots, component contracts, data bindings, interactions, states, asset keys, and handoff notes.",
        "Reference branch/node names from screenshots are visual context only. Actual definitions come from canonical research data.",
        "TopHudBar and SideNavigationRail belong to the App Shell and must not be duplicated in this Research screen record.",
        "Do not implement Vite/Roblox gameplay or export screen-spec content into public runtime."
      ]
    }),
    referenceViewport: "3840x2160",
...
export function screenHandoffText(record: ScreenDesignRecord, target: "Game Codex" | "Roblox Codex" = "Game Codex") {
  ...
  return [
    ...
    "",
    "Reference screen workflow:",
    record.references.length
      ? `- Reference screenshots: ${record.references.map((reference) => `${reference.id} (${reference.viewport})`).join(", ")}`
      : "- Reference screenshot required: upload the current Game screen capture in Studio before parity review.",
    "- Studio does not generate placeholder game layouts, panel grids, or client screen coordinates.",
    "- Game repository owns exact layout, CSS, responsive composition, rendering, and animation placement.",
    `- Studio reference viewport: ${record.referenceViewport}`,
    "",
    "Components:",
