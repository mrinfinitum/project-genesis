...
async function main() {
  ...
  assert(renderPage.includes("PlanetRenderEngineWorkspace"), "/render must render the Render Engine workspace.");
  assert(subrouteText.includes('redirect("/render")'), "Old Render subroutes must redirect to the new canonical Render workspace.");
  assert(workspace.includes("Render Engine"), "Render Engine title missing.");
  assert(workspace.includes("Canonical renderer definitions and Blender integration."), "Render Engine subtitle missing.");
  assert(workspace.includes("Surface Profiles") && workspace.includes("Cloud Profiles") && workspace.includes("Atmosphere Profiles"), "Technical profile groups missing.");
  assert(workspace.includes("Lighting Profiles") && workspace.includes("Camera Profiles") && workspace.includes("Output Profiles"), "Technical integration groups missing.");
  assert(workspace.includes("Render Contracts and Exports"), "Render contracts export panel missing.");
  assert(workspace.includes("Blender Mappings"), "Blender mapping panel missing.");
  assert(workspace.includes("Studio defines generation rules, render profiles, canonical defaults, validation, and exports."), "Architecture direction note missing.");
  assert(!workspace.includes("Planet Creator") && !workspace.includes("PlanetPreview") && !workspace.includes("PlanetDNA"), "Planet Creator presentation must be removed.");
  assert(!workspace.includes("Inspire Me") && !workspace.includes("Earth-like World") && !workspace.includes("Mountain Height"), "Creative planet-editing controls must be removed.");
  assert(!workspace.includes("Design living worlds. The renderer takes care of the rest."), "Planet Creator subtitle must be removed.");
